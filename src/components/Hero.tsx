...
const buttonStyles: Record<ActionButton['variant'], string> = {
  primary: 'bg-brandPurple hover:bg-brandPurple/90 text-white',
  secondary: 'bg-brandOrange hover:bg-brandOrange/90 text-white',
  github: 'bg-[#24292f] hover:bg-[#1b1f23] text-white',
              ...
              {actionButtons.map((button, idx) => {
                ...
                return (
                  <a
                    ...
                    rel={!isAnchor && !button.download ? 'noopener noreferrer' : undefined}
                    onClick={isAnchor ? (e) => {
                      e.preventDefault();
                      document.querySelector(button.href)?.scrollIntoView({ behavior: 'smooth' });
                    } : undefined}
                    className={`inline-flex items-center justify-center rounded-full px-6 py-2.5 font-medium transition-all duration-200 ${buttonStyles[button.variant]}`}
                  >
                    {button.label}
                  </a>
                );
          ...
          >
            {showAppStoreButton && appStoreHref && (
              <a
                ...
                className="flex items-center justify-center gap-3 w-[200px] h-12 rounded-[8px] border border-black bg-white hover:bg-gray-50 transition-colors"
              ...
              >
                ...
                <span className="flex flex-col leading-none text-left">
                  <span className="text-[9px] tracking-wide text-gray-800">Download on the</span>
                  <span className="text-[18px] font-semibold text-gray-800 -mt-px tracking-tight">App Store</span>
                </span>
              </a>
            )}
            {githubHref && (
              ...
              >
                <span className="flex flex-col leading-none text-right">
                  <span className="text-[9px] tracking-wide text-gray-800">View code on</span>
                  <span className="text-[18px] font-semibold text-gray-800 -mt-px tracking-tight">GitHub</span>
                </span>
            ...
            {webHref && (
              <Link
                to={webHref}
                className="flex items-center justify-center gap-3 w-[200px] h-12 rounded-[8px] bg-blue-500 hover:bg-blue-600 transition-colors"
                aria-label="View your portfolio on Opalite Web"
              >
                <span className="flex flex-col leading-none text-center">
                  ...
                  <span className="text-[18px] font-semibold text-white -mt-px tracking-tight">Log In</span>
                </span>
              </Link>
            )}
          </motion.div>
